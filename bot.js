...
//logger.info("Log4js is setup and working");
...
const executeTrade = async (_routerPath, _token0Contract, _token1Contract) => {
	...
	if (config.PROJECT_SETTINGS.isDeployed) {
		const transaction = await arbitrage
			.connect(account)
			.executeTrade(startOnUniswap, await _token0Contract.getAddress(), await _token1Contract.getAddress(), amount, { gasLimit: process.env.GAS_LIMIT });
