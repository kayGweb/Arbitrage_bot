...
contract MockUniswapV2Router02 {
    ...
    function setTokenSwapRate(address tokenA, address tokenB, uint256 rate) external {
        swapRates[tokenA][tokenB] = rate;
    }

    function getTokenSwapRate(address tokenA, address tokenB) external view returns (uint256) {
    ...
    function getAmountsOut(uint amountIn, address[] calldata path) external view returns (uint[] memory amounts) {
        ...
        if (swapRates[path[0]][path[1]] > 0) {
            ...
            if (path[0] == address(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2)) {
                // WETH has 18 decimals
                decimalsIn = 18;
            }
            
            if (path[1] == address(0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48)) {
                // USDC has 6 decimals
                decimalsOut = 6;
            }
            
            // Calculate based on rate and decimal adjustment
            amounts[1] = (amountIn * swapRates[path[0]][path[1]]) / (10**(decimalsIn - decimalsOut));
        } else if (swapRates[path[1]][path[0]] > 0) {
            // Calculate inverse rate
            uint8 decimalsIn = 18;
            uint8 decimalsOut = 18;
            
            if (path[1] == address(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2)) {
                // WETH has 18 decimals
                decimalsOut = 18;
            }
            
            if (path[0] == address(0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48)) {
                // USDC has 6 decimals
                decimalsIn = 6;
            }
            
            // Calculate inverse rate with decimal adjustment
            amounts[1] = (amountIn * (10**(decimalsOut - decimalsIn))) / swapRates[path[1]][path[0]];
        } else {
            // Default 1:1 rate
            amounts[1] = amountIn;
        }
    ...
    function getAmountsIn(uint amountOut, address[] calldata path) external view returns (uint[] memory amounts) {
        ...
        if (swapRates[path[0]][path[1]] > 0) {
            ...
            amounts[0] = (amountOut * (10**(decimalsIn - decimalsOut))) / swapRates[path[0]][path[1]];
        } else if (swapRates[path[1]][path[0]] > 0) {
            ...
            amounts[0] = amountOut * swapRates[path[1]][path[0]] / (10**(decimalsOut - decimalsIn));
        } else {
            // Default 1:1 rate
            amounts[0] = amountOut;
        }
...
contract MockBalancerVault {
    function flashLoan(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            require(tokens[i].transfer(address(recipient), amounts[i]), "Token transfer failed");
        }
